...
export default function StocksArchiveTable(props) {
  ...
  const columnsModifiedMasterLuxury = React.useMemo(() => {
    const findMasteColumnEditable = user?.modulesPermissionsMasterLuxury?.find(
      (item) => item?.moduleName === "inventory-sold-completed"
    );
    ...
    const handleChangeCheckBox = async (event, params) => {
      ...
      await axios
        .put(
          userActiveRole === "MLA"
            ? `https://api-dev.mlawatches.com/api/admin/stock/${params?.id}?tab=4`
            : `https://api-dev.mlawatches.com/api/admin/stock/masterLuxury/${params?.id}?tab=4`,
          payload,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const updatedColumns = React.useMemo(() => {
    return columnsByUser
      ?.map((col, index, arr) => {
        if (col.field === "model") {
          // Insert profit and profit_percentage after model
          return [
            col,
            arr?.find((item) => item.field === "profit"),
            arr?.find((item) => item.field === "profit_percentage"),
          ].filter(Boolean); // Filter out any undefined values
        }
        if (col?.field === "profit" || col?.field === "profit_percentage") {
          return null; // Remove them from their original position
        }
        return col;
      })
      .flat()
  ...
  const columnsModifiedMLA = React.useMemo(() => {
    ...
    if (user?.role === "SUPER_ADMIN") {
      return updatedColumns?.map((item) => {
    ...
    if (!inventoryCostModules) return updatedColumns;
  ...
  async function fetchData() {
    try {
      const response = await fetch(
        userActiveRole === "MLA"
          ? "https://api-dev.mlawatches.com/api/admin/stock?tab=4"
          : `https://api-dev.mlawatches.com/api/admin/stock/masterLuxury?tab=4`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${session}`,
  ...
  const processRowUpdate = async (newRow: GridRowModel) => {
    ...
    await fetch(
      userActiveRole === "MLA"
        ? `https://api-dev.mlawatches.com/api/admin/stock/${newRow.id}?tab=4`
        : `https://api-dev.mlawatches.com/api/admin/stock/masterLuxury/${newRow.id}?tab=4`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
