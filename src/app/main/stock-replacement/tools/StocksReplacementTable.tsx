...
export default function StocksReplacementTable(props) {
  ...
  const [pinnedColumns, setPinnedColumns] =
    React.useState<GridPinnedColumnFields>({
      left: ["rows_id", "status", "ny_la", "imageUrl", "id", "brand"],
    });
  ...
  React.useEffect(() => {
    ...
    //       "https://api-dev.mlawatches.com/api/admin/analytics/stock-replacement",
    //       {
    //         method: "GET",
    //         headers: {
    ...
    async function fetchDataNew() {
      try {
        const response = await fetch(
          "https://api-dev.mlawatches.com/api/admin/analytics/stock-replacement_new",
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${session}`,
  ...
  React.useEffect(() => {
    // Set an interval to refresh the page every 10 minutes (600,000 ms)
  ...
  //   newRow.minimum_wholesale_price_usd_only_mla = newRow.minimum_wholesale_price_usd_only_mla ? newRow.minimum_wholesale_price_usd_only_mla : 0;
  //   newRow.minimum_wholesale_price_usd = newRow.minimum_wholesale_price_usd ? newRow.minimum_wholesale_price_usd : 0;
  //   newRow.wholesale_price_usd = newRow.wholesale_price_usd ? newRow.wholesale_price_usd : 0;
  //   newRow.retail_price_usd = newRow.retail_price_usd ? newRow.retail_price_usd : 0;
  //   newRow.cost_usd = newRow.cost_usd ? newRow.cost_usd : 0;
  //   newRow.manual_max_price = newRow.manual_max_price ? newRow.manual_max_price : 0;
  //   newRow.manualmin_price = newRow.manualmin_price ? newRow.manualmin_price : 0;

  //   newRow.sold_price = newRow.sold_price ? newRow.sold_price : 0;
  //   newRow.wholesale_price_decreased_after_x_days_from_purchase_date = newRow.wholesale_price_decreased_after_x_days_from_purchase_date ? newRow.wholesale_price_decreased_after_x_days_from_purchase_date : 0;
  //   newRow.wholesale_price_usd = newRow.wholesale_price_usd ? newRow.wholesale_price_usd : 0;
  ...
  //   newRow.suggested_wholesale_price_usd_only_mla = newRow.suggested_wholesale_price_usd_only_mla ? newRow.suggested_wholesale_price_usd_only_mla : 0;
  //   newRow.total_amount_decreased_from_wholesale_price = newRow.total_amount_decreased_from_wholesale_price ? newRow.total_amount_decreased_from_wholesale_price : 0;
  //   newRow.w_s_price_decrease_every_n_days_starting_from_day_50 = newRow.w_s_price_decrease_every_n_days_starting_from_day_50 ? newRow.w_s_price_decrease_every_n_days_starting_from_day_50 : 0;

  //   newRow.column_r_min_price = newRow.column_r_min_price ? newRow.column_r_min_price : 0;
  //   newRow.range = newRow.range ? newRow.range : 0;
  //   newRow.column_s_wholesale_price = newRow.column_s_wholesale_price ? newRow.column_s_wholesale_price : 0;
  //   newRow.cost_for_min_w_price_formula = newRow.cost_for_min_w_price_formula ? newRow.cost_for_min_w_price_formula : 0;
  //   newRow.extra_300_for_rx_where_wholesale_price_20 = newRow.extra_300_for_rx_where_wholesale_price_20 ? newRow.extra_300_for_rx_where_wholesale_price_20 : 0;
  //   if (newRow.purchase_date != null && newRow.purchase_date != '') {
  //     newRow.days_from_purchase_date = calculateDayFromPurchaseDate(newRow.purchase_date)
  //   }
  ...
  return (
    <Box sx={{ height: "95vh", width: "100%", display: "flex" }}>
      {/* <div className="w-[30%]">
        <DataGridPro
          rows={rows}
          loading={loading}
