...
function updateRedirectUrl(user: PartialDeep<User>) {
    ...
    */
	...
	// }
	settingsConfig.loginRedirectUrl = `/`; // for example 'apps/academy'
}
...
export const selectUserShortcuts = (state: any) => state.user?.data?.shortcuts;

export const selectUserSettings = (state: any) => state.user?.data?.settings;
