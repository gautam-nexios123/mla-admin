...
const useJwtAuth = <User, SignInPayload, SignUpPayload>(
  ...
  const signIn = async (credentials: SignInPayload) => {
    const resIp = await fetch("https://api.ipify.org?format=json").then(
      (response) => response.json()
    );
    // const geoCode = await fetch(`http://www.geoplugin.net/json.gp?ip=${resIp.ip}`).then(response=> response.json())
    const response = axios.post(
      `https://api-dev.mlawatches.com/api/admin/auth/login`,
      {
        ...credentials,
        ip_address: resIp.ip,
        // location: geoCode.geoplugin_city +","+ geoCode.geoplugin_region +","+ geoCode.geoplugin_countryName,
      }
    );
    ...
    response.then(
      (res: AxiosResponse<any>) => {
        ...
        const refreshToken = res?.data?.results.refresh_token;
        if (res.data.statusCode == 200) {
          const response = axios
            .get(`https://api-dev.mlawatches.com/api/admin/auth/me`, {
              headers: {
                Authorization: `Bearer ${accessToken}`,
              },
            })
            .then((user) => {
              userData = user.data.results;
              console.log(userData);

              handleSignInSuccess(userData, accessToken, refreshToken);
              return userData;
            });
        } else {
